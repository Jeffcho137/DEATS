...
export function Order_search (props) {
      ...
      fetch('https://deats-backend-test.herokuapp.com/my_deliverer/',
        ...
        .then((data) => {
            ...
            if (data.succeeded == true) {
              setDelInfo(data.deliverer_info)
              setDelId(data.deliverer_id)
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.centeredView}>
                <View style={styles.modalView}>
                  <View style={styles.modal_text}>
                    <Text style={styles.modalText}>{delId ? "Deliverer found!" : "Sorry, no deliverer is available yet"}</Text>
                    <Text style={styles.deliverer_is}> {delId ? "Your deliverer is " + delInfo.name : "Check again later"}</Text>
                    <Text style={styles.deliverer_is}> {delId ? "Delivery fee: $3.00" : ""} </Text>
                  </View>
                  <Pressable
                      style={[styles.button, styles.buttonClose]}
                      onPress={() => {setModalVisible(!modalVisible); {delId ? props.navigation.navigate('OrderMatch', 
                      {
                        delId: delId,
                        delInfo: delInfo,
                  ...
                  >
                    <Text style={styles.textStyle}> {delId ? "Accept Delivery" : "Go back"} </Text>
                  </Pressable>
                  <Pressable
                      onPress={() => {setModalVisible(!modalVisible); props.navigation.navigate('Home')}}
                  >
            ...
            <Button color="#006400" title="Cancel" onPress={() => props.navigation.navigate('Home')}></Button>
...
// export class Order_searchC extends Component {
//   constructor(props){
//     super(props)
//     this.state = {
